...
struct Options {
  ...
  std::string certificate_path = "erdos848_certificate.tsv";
  bool quiet = false;
};
...
struct PrimeBlockState {
  RootInfo root;
  std::vector<int> plus_witness_values;
  std::vector<int> minus_witness_values;
  std::vector<WitnessRow> plus_rows;
  std::vector<WitnessRow> minus_rows;
...
std::vector<PrimeBlockState> build_prime_blocks(int n_max, bool quiet) {
  ...
  for (std::size_t i = 0; i < roots.size(); ++i) {
    ...
    block.root = roots[i];
    block.plus_witness_values = build_witness_values_for_root(roots[i].r_plus, roots[i].p2, n_max, roots, i);
    block.minus_witness_values = build_witness_values_for_root(roots[i].r_minus, roots[i].p2, n_max, roots, i);
    block.plus_rows.resize(block.plus_witness_values.size());
    block.minus_rows.resize(block.minus_witness_values.size());
    for (std::size_t j = 0; j < block.plus_witness_values.size(); ++j) {
      block.plus_rows[j].x = block.plus_witness_values[j];
    }
    for (std::size_t j = 0; j < block.minus_witness_values.size(); ++j) {
      block.minus_rows[j].x = block.minus_witness_values[j];
    }
    if (!quiet) {
      std::cerr << "Prepared p=" << block.root.p
                << " with witness outsiders +=" << block.plus_rows.size()
                << " -=" << block.minus_rows.size() << "\n";
    }
...
void handle_new_plus_witness(
  ...
  auto &row = block.plus_rows[block.next_plus_witness];
  ...
  for (std::size_t j = 0; j < block.next_minus_witness; ++j) {
    const i64 value = 1LL * row.x * block.minus_rows[j].x + 1;
    if (is_non_squarefree_value(value, prime_squares)) {
      ++row.degree;
      ++block.minus_rows[j].degree;
      block.current_dmax = std::max(block.current_dmax, block.minus_rows[j].degree);
    }
  }
...
void handle_new_minus_witness(
    ...
    const std::vector<int> &mask_primes,
    const std::vector<i64> &prime_squares) {
  auto &row = block.minus_rows[block.next_minus_witness];
  row.exchange_count = count_non_squarefree_progression(25LL * row.x, 7LL * row.x + 1, base_count, mask_primes);
  row.degree = 0;
  for (std::size_t i = 0; i < block.next_plus_witness; ++i) {
    const i64 value = 1LL * row.x * block.plus_rows[i].x + 1;
    if (is_non_squarefree_value(value, prime_squares)) {
      ++row.degree;
      ++block.plus_rows[i].degree;
      block.current_dmax = std::max(block.current_dmax, block.plus_rows[i].degree);
    }
  }
  block.current_smax = std::max(block.current_smax, row.exchange_count);
  block.current_dmax = std::max(block.current_dmax, row.degree);
...
void advance_base_event(
  ...
  for (auto &block : blocks) {
    for (std::size_t i = 0; i < block.next_plus_witness; ++i) {
      const i64 value = 1LL * block.plus_rows[i].x * y + 1;
      if (is_non_squarefree_value(value, prime_squares)) {
        ++block.plus_rows[i].exchange_count;
        block.current_smax = std::max(block.current_smax, block.plus_rows[i].exchange_count);
      }
    }
    for (std::size_t i = 0; i < block.next_minus_witness; ++i) {
      const i64 value = 1LL * block.minus_rows[i].x * y + 1;
      if (is_non_squarefree_value(value, prime_squares)) {
        ++block.minus_rows[i].exchange_count;
        block.current_smax = std::max(block.current_smax, block.minus_rows[i].exchange_count);
      }
    }
  }
}
...
int main(int argc, char **argv) {
  try {
    ...
    auto blocks = build_prime_blocks(opts.n_max, opts.quiet);
    ...
    while (e < events.size()) {
      ...
      while (e < events.size() && events[e].n == n) {
        ...
        if (ev.type == EventType::Base) {
          ++current_base_count;
          advance_base_event(blocks, n, prime_squares);
          changed = true;
        } else {
          auto &block = blocks[static_cast<std::size_t>(ev.p_index)];
          if (ev.type == EventType::RootPlus) {
            ++block.current_raw_plus;
            while (block.next_plus_witness < block.plus_witness_values.size() &&
                   block.plus_witness_values[block.next_plus_witness] == n) {
              handle_new_plus_witness(block, current_base_count, mask_primes, prime_squares);
            }
          } else {
            ++block.current_raw_minus;
            while (block.next_minus_witness < block.minus_witness_values.size() &&
                   block.minus_witness_values[block.next_minus_witness] == n) {
