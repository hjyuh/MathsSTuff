...
struct Options {
  ...
  std::string certificate_path = "erdos848_certificate_v3.tsv";
  bool quiet = false;
};
...
void update_range_best(
    std::vector<int> &best_margin,
    std::vector<int> &best_p,
    int start_n,
    int end_n,
    int margin,
    int p) {
  if (start_n > end_n || margin >= std::numeric_limits<int>::max() / 8) {
    return;
  }
  if (start_n < 2) {
    start_n = 2;
  ...
  for (int n = start_n; n <= end_n; ++n) {
    if (margin < best_margin[static_cast<std::size_t>(n)]) {
      best_margin[static_cast<std::size_t>(n)] = margin;
      best_p[static_cast<std::size_t>(n)] = p;
    }
  }
}
...
int main(int argc, char **argv) {
  try {
    ...
    std::vector<int> best_margin(static_cast<std::size_t>(opts.n_max) + 1, std::numeric_limits<int>::max() / 4);
    std::vector<int> best_p(static_cast<std::size_t>(opts.n_max) + 1, 0);
    ...
    for (std::size_t p_index = 0; p_index < roots.size(); ++p_index) {
      ...
      while (true) {
        const int next_global_n = (next_global_root < global_root_events.size())
            ? global_root_events[next_global_root].n
            : std::numeric_limits<int>::max();
        const int n = std::min(std::min(next_base_n, next_plus_raw_n), std::min(next_minus_raw_n, next_global_n));
        if (n == std::numeric_limits<int>::max()) {
          break;
        }

        update_range_best(best_margin, best_p, previous_n + 1, n - 1, current_margin, root.p);
        ...
        update_range_best(best_margin, best_p, n, n, current_margin, root.p);
        previous_n = n;

        if (!opts.quiet && (n % 100000 == 0 || n == opts.n_max)) {
          std::cerr << "Block p=" << root.p << " reached N=" << n
                    << " margin=" << current_margin
                    << " M=" << current_base_count << "\n";
        }
      }

      update_range_best(best_margin, best_p, previous_n + 1, opts.n_max, current_margin, root.p);
    }
    ...
    for (int n = 2; n <= opts.n_max; ++n) {
      const int base_count = (n >= 7) ? ((n - 7) / 25 + 1) : 0;
      const int margin = (best_margin[static_cast<std::size_t>(n)] >= std::numeric_limits<int>::max() / 8)
          ? (base_count + 1)
          : best_margin[static_cast<std::size_t>(n)];
      const int p = best_p[static_cast<std::size_t>(n)];
      const char *status = (margin > 0) ? "PASS" : "FAIL";
      out << n << '\t' << status << '\t' << margin << '\t' << p << '\t' << base_count << '\n';
    }

    const int final_base_count = (opts.n_max >= 7) ? ((opts.n_max - 7) / 25 + 1) : 0;
    const int final_margin = (best_margin[static_cast<std::size_t>(opts.n_max)] >= std::numeric_limits<int>::max() / 8)
        ? (final_base_count + 1)
        : best_margin[static_cast<std::size_t>(opts.n_max)];
    const int final_p = best_p[static_cast<std::size_t>(opts.n_max)];
